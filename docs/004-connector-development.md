...
- 華南登入頁的帳號欄位在 parser-blocking 腳本之後。Browser Run 若在載入那些腳本時停住，live view 會一直空白，且 `USERIDTEXT` 不會出現。此情況必須記導覽狀態、`readyState`、html 長度與是否出現登入欄位，並以連線失敗處理，不得當成驗證碼失敗請使用者人工輸入。
